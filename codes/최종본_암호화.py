...
def scramble(li, random_key):
    ...
    for i in range(routine):
        ...
        if R in random_key:
            ...
            list2[33:36] = list1[35], list1[32], list1[30]
...
#원본 이미지 열기(인식)     
img = Image.open('학술제 코딩\image.bmp') #문자열 안에 파일 경로 입력
...
final= np.array(newlist).reshape((1920, 1080))

img = Image.fromarray(final)
deg_image = img.transpose(Image.ROTATE_90)
